                    ...
                    User.find({} ,'-children -events -sentProjects -assignedProjects -receivedProjects -unit -area -profileUrl' ,(err ,usersList)=>{
                        ...
                        if(req.query.sendTo == undefined){
                            ...
                            User.findById(req.query.sendTo ,(err ,sendTo)=>{
                                if(err){throw err}
                                res.render("users/newMail" ,{user: user ,units: units ,sendTo: sendTo,usersList: JSON.stringify(usersList)});
                            });
                        }
...
router.get("/users/:id/mails/received" ,(req ,res)=>{
    User.findById(req.params.id ,'-children -events')
    .populate({
        path: 'receivedMails',
        populate: {
            path: 'sent_by',
        ...
        if(err){throw err;}
            ...
            Unit.find().exec((err ,units)=>{
                if(err){throw err;}
                    ...
                    res.render("users/receivedMails" ,{user: user ,units: units ,receivedMails: receivedMails});
...
router.get("/users/:id/mails/sent" ,(req ,res)=>{
    ...
    .populate({
        ...
        populate: {
            ...
            select: '-children -events -sentMessages -receivedMessages -sentProjects -assignedProjects -receivedProjects -unit -area -profileUrl -sentMails -receivedMails -contacts'
        }
    })
    .exec((err ,user)=>{
        if(err){throw err;}
            ...
            Unit.find().exec((err ,units)=>{
                if(err){throw err;}
                    ...
                    res.render("users/sentMails" ,{user: user ,units: units});
                    // console.log(user.sentMails);
                }
            });
        }
    });
        ...
        if(err){throw err;}
            ...
            Unit.find().exec((err ,units)=>{
                if(err){throw err;}
                    ...
                    User.find({} ,'-children -events -sentMessages -receivedMessages -sentProjects -assignedProjects -receivedProjects -unit -area -profileUrl' ,(err ,usersList)=>{
                        if(err){throw err;}
                        Mail.findById(req.params.mailId).populate({
                            path: 'sent_by',
                            select: '-children -events -sentMessages -receivedMessages -sentProjects -assignedProjects -receivedProjects -unit -area -profileUrl -sentMails -receivedMails -contacts'
                            ...
                            res.render("users/receivedMailContent" ,{user: user ,units: units ,mail: mail ,usersList: JSON.stringify(usersList)});
                            // console.log(mail);
                        });
                    });
                }
            ...
            Unit.find().exec((err ,units)=>{
                if(err){throw err;}
                    ...
                    Mail.findById(req.params.mailId).populate([{
                        ...
                        res.render("users/newMailReply" ,{user: user ,units: units ,mail: mail});
                        // console.log(mail);
                    });
                }
            });
            ...
            Unit.find().exec((err ,units)=>{
                if(err){throw err;}
                    ...
                    .exec((err , mail)=>{
                        ...
                        res.render("users/sentMailReplies" ,{user: user ,units: units ,mailId: req.params.mailId ,replies: replies});
                        // console.log(replies);
                    });
                }
            });
