var params = {
  ...
  d3.json(params.dataLoadUrl, function(data) {
      var myData =  document.getElementById("gg").textContent;
      params.data = JSON.parse(myData);
      params.pristinaData = JSON.parse(JSON.stringify(myData));
  ...
  function drawOrganizationChart(params) {
    ...
    dynamic.rootNodeLeftMargin = 381;
    ...
    var svg = d3.select(attrs.selector)
      .append("svg")
      .attr("width", attrs.width / 2)
      .attr("height", attrs.height)
      .call(zoomBehaviours)
      .append("g")
      .attr("transform", "translate(" + 381 + "," + 65 + ")");
    ...
    function update(source, param) {
      ...
      nodeGroup.append("text")
        ...
        nodeGroup.append("text")
        .attr("x", dynamic.nodeTextLeftMargin + 85)
        .attr("y", attrs.nodePadding + 10)
        .attr('class', 'emp-assigned')
        .attr("text-anchor", "left")
        .text(function(d) {
          if(d.assigned == true){
            return "(Assigned)";
      ...
      function tooltipContent(item) {
        ...
        strVar += "      <a class=\"name\" href='/users/" + item.user + "/profil' target=\"_blank\"> " + item.name + "<\/a>";
        // if(item.assigned == true){
        //   // strVar += "      <span style = 'color: orange;'>Assigned</span>";
        //   console.log(item.name + " is assigned");
        // }
