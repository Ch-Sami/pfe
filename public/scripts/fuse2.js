$(function (window) {
    ...
    App.Main = new (Fiber.extend(function () {
      return {
        ...
        setupNodes: function () {
          this.$itemsTextArea1 = $('#itemsTextArea1')
          this.$searchTextbox1 = $('#searchTextbox1')
          this.$resultTextArea1 = $('#resultTextArea1')
          this.$jsTextArea1 = $('#jsTextArea1')
          this.$searchTimeLabel1 = $('#searchTimeLabel1')
        },
        bindEvents: function () {
          this.options = new App.Options()
          this.options.on('change', _.bind(this.setupFuse, this))
          this.$itemsTextArea1.on('change', _.bind(this.setupItems, this))
  
          this.$searchTextbox1.on('keyup', _.debounce(_.bind(function () {
            this.search(this.$searchTextbox1.val())
          }, this), 0))
  
          this.$itemsTextArea1.on('change', function () {
            ga('send', 'event', 'Demo', 'change', 'items')
          })
          this.$searchTextbox1.on('change', function () {
            ga('send', 'event', 'Demo', 'change', 'search')
          })
        },
        setupItems: function () {
          var list = this.$itemsTextArea1.val()
          this.list = eval(list)
          this.setupFuse()
        },
        setupFuse: function () {
        ...
        search: function (pattern) {
          if (pattern.length > this.options.data.maxPatternLength) {
            this.$resultTextArea1.html('Pattern length is too long')
            return
          }
          ...
          this.$searchTimeLabel1.text((end - start) + ' ms')
          this.$resultTextArea1.text(JSON.stringify(result, null, '  '))
          this.updateJS()
        },
        updateJS: function () {
          var arr = []
